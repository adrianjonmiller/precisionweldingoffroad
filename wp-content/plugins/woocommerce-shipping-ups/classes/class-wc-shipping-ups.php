...
class WC_Shipping_UPS extends WC_Shipping_Method {
	...
	public function clear_transients() {
		delete_transient( 'wc_ups_quotes' );
	}
    ...
    public function calculate_shipping( $package ) {
    	...
    	$rates            = array();
    	$ups_responses	  = array();
    	$results          = get_transient( 'wc_ups_quotes' );
    	libxml_use_internal_errors( true );

		// Only return rates if the package has a destination including country, state, postcode
		if ( ( ''==$package['destination']['country'] ) || ( ''==$package['destination']['state'] ) || ( ''==$package['destination']['postcode'] ) ) {
			if ( $this->debug ) {
				$woocommerce->add_message( __('UPS: Country, State or Zip not yet supplied. Rates not requested.', 'wc_ups') );
			}
			return; 
		}
    	...
    	if ( $package_requests ) {
			...
			foreach ( $rate_requests as $code => $request ) {
	    		...
	    		if ( $this->debug ) {
					...
					$woocommerce->add_message( 'UPS RESPONSE: <pre>' . print_r( htmlspecialchars( $response['body'] ), true ) . '</pre>' );
				}
		...
		// Set transient
		set_transient( 'wc_ups_quotes', $results );
